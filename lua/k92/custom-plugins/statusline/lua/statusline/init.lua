...
function M.diff_status()
  ...
  if changes.add > 0 then
    table.insert(changes_str, string.format("+%s", changes.add))
  end
  if changes.delete > 0 then
    table.insert(changes_str, string.format("-%s", changes.delete))
  end
  if changes.change > 0 then
    table.insert(changes_str, string.format("~%s", changes.change))
  end

  if #changes_str == 0 then
    return ""
  end

  return table.concat(changes_str, " ") .. " "
end
...
function M.have_left_component()
...
function M.setup(user_config)
  ...
  local eval_have_left_component = "luaeval('(require(''statusline'').have_left_component())')"
  ...
  local left_component = string.format(
    "%s ? %s .. '%%=' : '' ",
    eval_have_left_component,
    table.concat({ eval_git_status, eval_diff_status, eval_warp_status }, " .. ")
  )
